...
class Level:
    def __init__(self, window: Surface, name: str, game_mode: str, player_score: list[int]):
        # Dobrar o timeout para o Level 3
        if name == 'Level3':
            self.timeout = TIMEOUT_LEVEL * 2  # Level 3 dura o dobro dos outros níveis
        else:
            self.timeout = TIMEOUT_LEVEL
    ...
    def run(self, player_score: list[int]):
        ...
        while True:
            ...
            for event in pygame.event.get():
                ...
                # Gerar apenas inimigos do tipo Enemy3 no Level 3
                if event.type == EVENT_ENEMY:
                    if self.name == 'Level3':
                        # Gerar apenas Enemy3 no Level3
                        self.entity_list.append(EntityFactory.get_entity('Enemy3'))
                    else:
                        # Gerar Enemy1 ou Enemy2 nos outros níveis
                        choice = random.choice(('Enemy1', 'Enemy2'))
                        self.entity_list.append(EntityFactory.get_entity(choice))
